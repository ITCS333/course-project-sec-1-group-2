...
function updateAssignment(PDO $db, array $data): void
{
    ...
    if (empty($setClauses)) {
        sendResponse(['success' => false, 'message' => 'No valid fields provided to update.'], 400);
    }
...
function createComment(PDO $db, array $data): void
{
    ...
    if (empty($assignmentId) || $author === '' || $text === '') {
    ...
    $stmt->execute([(int)$assignmentId, sanitizeInput($author), sanitizeInput($text)]);
